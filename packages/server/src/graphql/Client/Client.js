...
export const resolvers = {
    ...
    clients: async (_, args) => {
      ...
      if (sort) {
        clients.sort((clientA, clientB) => {
          ...
          if (sort.sortment === ListSortmentEnum.DESC)
  ...
  Mutation: {
    ...
    enableClient: async (_, { id }) => {
      ...
      if (!currentClient) throw new Error(`Client "${id}" is already enabled`);
    ...
    disableClient: async (_, { id }) => {
      ...
      if (!currentClient) throw new Error(`Client "${id}" is already disabled`);
